...
//void CalculateAverageDistances(const vector<Point>& contour, const Point2f& center, double& averageDistance_1, double& averageDistance_2, Point2f point_1, Point2f point_2, Mat& contourImage)
//{
...
//        double distance = norm(Point2f(static_cast<float>(point.x), static_cast<float>(point.y)) - Point2f(static_cast<float>(center.x), static_cast<float>(center.y)));
//
...
int test_create_folder() {
    FILE* tempFile;
    errno_t err = fopen_s(&tempFile, "test.txt", "w+");
    if (err != 0)
    {
#ifdef VERBOSE
...
int AcquireImages(CallbackFunction callback)
{
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (contours[closestContourIndex].size() >= 5) { // fitEllipse requires at least 5 points
                        ...
                        wings_detection = DetectWings(binaryImage, tail_point, contourImage);
                        }
                    ...
                    cv::imencode(".jpg", display_image_trash, encodedImage); // Encode the Mat as JPEG

                    struct_result.display_image_size = encodedImage.size();
                    struct_result.display_image = new unsigned char[struct_result.display_image_size];
                    std::memcpy(struct_result.display_image, encodedImage.data(), struct_result.display_image_size);

					struct_result.headingAngle = headingAngle; // convert in rd
					struct_result.frame_rate = frame_rate;
                    if (wings_detection == true) {
						struct_result.wings = 1;
					}
                    else
                    {
                        struct_result.wings = 0;
                    }
                    if (callback)
                    {
						callback(struct_result);
					}
                    ...
                    // ** SAVING STEP ** //
                    if (SAVE_IMAGE == "TRUE") {
                        SaveImage(PATH_SAVE_IMG, deviceSerialNumber, display_image, iteration);
                    }
